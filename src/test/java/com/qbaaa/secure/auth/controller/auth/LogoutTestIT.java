...
class LogoutTestIT {
    ...
    @ParameterizedTest(name = "#{index} - Run test with domainName = {0},  username = {1}, password = {2}")
    ...
    void shouldLogout(String domainName, String username, String password) {
        try {
            ...
            // when
            mockMvc.perform(MockMvcRequestBuilders.post(API_POST_LOGOUT, domainName)
    ...
    void shouldReturnErrorBadDomainLogout() {
        try {
            ...
            // when
            mockMvc.perform(MockMvcRequestBuilders.post(API_POST_LOGOUT, "test-domain")
                            .header(HttpHeaders.AUTHORIZATION, "Bearer " + token.accessToken())
                            .content(objectMapper.writeValueAsString(refreshTokenRequest))
                            .contentType(MediaType.APPLICATION_JSON_VALUE))
