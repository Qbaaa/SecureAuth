...
@ActiveProfiles("test")
class EmailTokenCleanupJobTestIT {

  @Autowired private EmailTokenCleanupJob emailTokenCleanupJob;
  ...
  void shouldDeletedExpiredToken() {
    try {
      ...
      assertAll(
          "CHECK TABLES DATA BEFORE JOB DELETE EMAIL TOKEN",
          () -> Assertions.assertEquals(5, emailTokenRepositoryTest.count()));

      // when
      emailTokenCleanupJob.deleteExpiredTokens();

      // then
      assertAll(
          "CHECK TABLES DATA AFTER JOB DELETE EMAIL TOKEN",
          () -> Assertions.assertEquals(2, emailTokenRepositoryTest.count()));
