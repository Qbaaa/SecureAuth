...
import com.qbaaa.secure.auth.auth.infrastructure.job.SessionCleanupJob;
...
@ActiveProfiles("test")
class SessionCleanupJobTestIT {

  @Autowired private SessionCleanupJob sessionCleanupJob;

  @Autowired private FakeTimeProvider fakeTimeProvider;

  @Autowired private SessionRepositoryTest sessionRepository;
  ...
  void shouldDeletedExpiredToken() {
    try {
      ...
      assertAll(
          "CHECK TABLES DATA BEFORE JOB DELETE SESSION",
          () -> Assertions.assertEquals(4, sessionRepository.count()));

      // when
      sessionCleanupJob.deleteExpiredTokens();

      // then
      assertAll(
          "CHECK TABLES DATA AFTER JOB DELETE SESSION",
