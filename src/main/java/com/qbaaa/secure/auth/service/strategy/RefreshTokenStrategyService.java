...
public class RefreshTokenStrategyService extends AuthStrategy {
    ...
    public TokenResponse authenticate(String domainName, String baseUrl, AuthRequest request) {
        ...
        var authenticated = jwtService.verify(issuer, refreshTokenOld)
        ...
        final var user = userService.findUserBySession(sessionUuid)
        ...
        final var claimJwtDto = new ClaimJwtDto(domainName, user.getUsername(), user.getEmail(),
                roles, baseUrl, refreshTokenOld, configDomain.getAccessTokenValidity());
        final var accessToken = jwtService.createAccessToken(claimJwtDto);
        final var refreshTokenNew = jwtService.createRefreshToken(baseUrl, domainName, session,
                configDomain.getRefreshTokenValidity());

        refreshTokenService.deleteRefreshToken(refreshTokenOld);
        refreshTokenService.createRefreshToken(user, configDomain.getRefreshTokenValidity(), refreshTokenNew);
